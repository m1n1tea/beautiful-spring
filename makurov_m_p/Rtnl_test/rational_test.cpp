...
TEST_CASE("comparison operators") {
	...
	// не проверел другие операторы, поскольку они реализованны через данные
...
TEST_CASE("input check") {
	CHECK(testIn("1/2 ")); // можно ли жить без пробела(любого друго сепаратора) в конце? если да, то как? не знаю зачем правда
	CHECK(!(testIn("1/-2 ")));
	CHECK(testIn("-1/2 "));
	CHECK(!(testIn("-1/-2 ")));
	CHECK(testIn("5/10 "));
