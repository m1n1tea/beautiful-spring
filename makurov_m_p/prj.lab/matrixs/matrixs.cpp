...
MatrixS::MatrixS(const ptrdiff_t size_y, const ptrdiff_t size_x) : size_x_(size_x), size_y_(size_y), matrix_(ArrayT<int>(size_x* size_y)), rows_(ArrayT<ptrdiff_t>(size_y)) {
    for (ptrdiff_t i = 1; i < size_y_; ++i) {
        rows_[i] = i;
    }

}

MatrixS::MatrixS(const SizeType& size) :size_x_(get<1>(size)), size_y_(get<0>(size)), matrix_(ArrayT<int>(get<0>(size)* get<1>(size))), rows_(ArrayT<ptrdiff_t>(get<0>(size)))
{
...
int& MatrixS::at(const ptrdiff_t ind_y, const ptrdiff_t ind_x) {
    indexCheck({ ind_y,ind_x });
    return  matrix_[rows_[ind_y] * size_x_ + ind_x];
}

const int& MatrixS::at(const ptrdiff_t ind_y, const ptrdiff_t ind_x) const {
    indexCheck({ ind_y,ind_x });
    return  matrix_[rows_[ind_y] * size_x_ + ind_x];
}
...
void MatrixS::resize(const ptrdiff_t size_y_new, const ptrdiff_t size_x_new) {
    sizeCheck(size_y_new);
    sizeCheck(size_x_new);
    ptrdiff_t sizeXSmall = (size_x_new < size_x_) ? size_x_new : size_x_;
    ptrdiff_t sizeYSmall = (size_y_new < size_y_) ? size_y_new : size_y_;
    ...
    for (ptrdiff_t i = 0; i < size_y_new; i++)
            rows_[i] = i;
