...
void MatrixS::resize(const std::ptrdiff_t size_y_new, const std::ptrdiff_t size_x_new) {
    ...
    std::ptrdiff_t size_x_small = (size_x_new < size_x_) ? size_x_new : size_x_;
    std::ptrdiff_t size_y_small = (size_y_new < size_y_) ? size_y_new : size_y_;
    ArrayT<int> newMatrix = ArrayT<int>(size_x_new * size_y_new);


    for (std::ptrdiff_t y = 0; y < size_y_small; ++y) {
        for (std::ptrdiff_t x = 0; x < size_x_small; ++x) {
